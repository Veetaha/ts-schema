...
/**
 * Determines whether the specified `suspect` value shape satisfies the 
 * restriction of the given `schema`.
 * 
 ...
 * }  
 * 
 * const HumanSchema = Tsv.schema({  // noop function that preserves unit types
 *     name: 'string',
 *     id:   'number'
 * });
 * 
 * // generate static TypeScript type:
 * type Human = Tsv.UnpackSchema<typeof HumanSchema>;
 * 
 * // type Human === {              
 * //     name: string;
 * //     id:   number;
 ...
 *     if (Tsv.test(HumanSchema, maybeHuman)) {
 *         // maybeHuman is of type that is assignable to Human here
 *         // it is inferred to be Tsv.UnpackSchema<typeof HumanSchema> exactly
 *         maybeHuman.name;
 *         maybeHuman.id;
 *     }
 * }
 ...
 *   const ExpectedJsonSchema: Tsv.Schema = {}; //some type schema
 *   const dbEntity = {}; // some database entity
 *
 *   const mismatch = Tsv.validate(ExpectedJsonSchema, untrustedJson);
 *   if (mismatch != null) {
 *       console.log(
 *           mismatch.path,
 ...
 *       throw new Tsv.TypeMismatchError(mismatch);
 *   }
 ...
 */
export function validateOrFail(...args: Parameters<typeof validate>) {
